...
unsigned char mcu_dp_raw_update(unsigned char dpid,const unsigned char value[],unsigned short len)
{
    ...
    if(stop_update_flag == SDK_ENABLE)
        return SDK_SUCCESS;
    //
...
unsigned char stream_trans_send(unsigned int id, unsigned char *buffer, unsigned long buf_len)
{
    ...
    for (int cnt = 0; cnt < pack_num; cnt++,map_offset += this_len) {
        ...
        if(SDK_ERROR == stream_trans(id, map_offset, buffer + map_offset, this_len)) {
            //mcu正在升级中，不可以进行流服务传输
            //printf("is upgrade\n");
            return SDK_ERROR;
        }
...
unsigned char maps_stream_trans_send(unsigned int id, unsigned char sub_id, unsigned char sub_id_pro_mode, unsigned char *buffer, unsigned long buf_len)
{
    ...
    for (int cnt = 0; cnt < pack_num; cnt++,map_offset += this_len) {
        ...
        if(SDK_ERROR == maps_stream_trans(pro_ver, id, sub_id, sub_id_pro_mode, map_offset, buffer + map_offset, this_len)) {
            //mcu正在升级中，不可以进行流服务传输
            //printf("is upgrade\n");
            return SDK_ERROR;
        }
